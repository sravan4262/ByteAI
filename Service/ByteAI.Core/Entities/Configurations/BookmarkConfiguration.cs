...
public sealed class BookmarkConfiguration : IEntityTypeConfiguration<Bookmark>
{
    public void Configure(EntityTypeBuilder<Bookmark> builder)
    {
        ...
        builder.HasOne(b => b.Byte).WithMany(by => by.Bookmarks)
            .HasForeignKey(b => b.ByteId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(b => b.User).WithMany(u => u.Bookmarks)
            .HasForeignKey(b => b.UserId).OnDelete(DeleteBehavior.Cascade);

        builder.HasIndex(b => b.UserId).HasDatabaseName("ix_bookmarks_user_id");
    }
}
