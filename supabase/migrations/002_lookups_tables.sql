...
-- subdomain_id is nullable: user-created tech stacks (added on the fly while
-- posting a byte) have no curated subdomain. ON DELETE SET NULL ensures a
-- subdomain removal doesn't cascade-delete uncategorised user stacks.
CREATE TABLE IF NOT EXISTS lookups.tech_stacks (
    id           uuid    PRIMARY KEY DEFAULT gen_random_uuid(),
    subdomain_id uuid    REFERENCES lookups.subdomains(id) ON DELETE SET NULL,
    name         text    NOT NULL UNIQUE CHECK (char_length(name) BETWEEN 1 AND 100),
    label        text    NOT NULL CHECK (char_length(label) BETWEEN 1 AND 100),
    sort_order   integer NOT NULL DEFAULT 0
);
CREATE INDEX IF NOT EXISTS ix_tech_stacks_subdomain_id ON lookups.tech_stacks (subdomain_id);
COMMENT ON TABLE lookups.tech_stacks IS 'Lookup: tech stack items grouped by subdomain. subdomain_id is nullable for user-created stacks.';
