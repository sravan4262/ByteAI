...
const lineStyle: Record<string, { color: string; prefix: string }> = {
  ...
  output:  { color: 'text-[var(--t2)]',     prefix: ' ' },
    ...
    <div className="flex-1 overflow-y-auto px-4 py-3 space-y-0.5 scrollbar-thin scrollbar-thumb-[var(--border-m)] scrollbar-track-transparent">
      {lines.map(line => {
        if (line.type === 'record' && line.meta) {
          ...
          const sColor = statusColor[line.meta.status] ?? 'text-[var(--t3)]'
          return (
            <div
              key={line.id}
              className={`flex items-center gap-2 font-mono text-[11px] leading-relaxed py-0.5 ${ENTRY_ANIM}`}
            >
              <span className="flex-shrink-0 w-3 text-center opacity-40 mt-px"> </span>
              <span className={`px-1.5 py-px rounded text-[9px] border font-semibold tracking-wide flex-shrink-0 ${badge.cls}`}>
                {badge.label}
              </span>
              <span className="text-[var(--t2)] truncate flex-1">{line.text}</span>
              <span className={`flex-shrink-0 text-[9px] ${sColor}`}>{line.meta.status}</span>
              <span className="flex-shrink-0 text-[9px] text-[var(--t3)]">{line.meta.date}</span>
            </div>
          )
        }
        ...
        return (
          <div
            key={line.id}
            className={`flex items-start gap-2 font-mono text-[11px] leading-relaxed ${style.color} ${ENTRY_ANIM}`}
          >
